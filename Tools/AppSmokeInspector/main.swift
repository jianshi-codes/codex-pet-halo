...
guard ownedWindows.count <= 1 else {
    fputs("error: expected at most one visible Halo window\n", stderr)
    exit(1)
}

if let window = ownedWindows.first {
    guard let bounds = window[kCGWindowBounds as String] as? [String: NSNumber],
...
print(ownedWindows.isEmpty ? "Halo window: safely hidden" : "Halo window: one supported panel")
