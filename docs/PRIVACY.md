...
Decoded M4 values must be finite, normalized coordinates must remain in `0...1`, offsets are capped at 10,000 points, and unsupported versions are ignored. Startup idempotently deletes the legacy `io.github.jianshicodes.PetHalo.petFollowing.anchor.v1` key so old Pet offsets cannot affect placement; it does not read, rewrite, or delete the M4 fallback anchor. Pet coordinates, selected side, activity geometry, PID, AX element, title, identifier, geometry snapshot, screen index, executable path, account field, and Usage value are never persisted.
