...
Decoded M4 values must be finite, normalized coordinates must remain in `0...1`, offsets are capped at 10,000 points, and unsupported versions are ignored. The visual-center offset must be finite and remain within half the Pet Ring panel diameter. Startup idempotently deletes the legacy `io.github.jianshicodes.PetHalo.petFollowing.anchor.v1` key without deleting the M4 anchor or visual offset. Pet coordinates, dialog orientation, activity geometry, PID, AX element, title, identifier, geometry snapshot, screen index, executable path, account field, and Usage value are never persisted.
