...
- `HaloPanelController` applies surface-specific size, shadow, and mouse policy. Pet Ring is `252×252`, shadowless, normally click-through, non-activating, and cannot become key or main. Only explicit visual-center calibration temporarily accepts dragging.
...
The fixed geometry uses three independent SwiftUI vector tracks and progress arcs at 104/94/84-point radii with 6-point strokes and a 162-point transparent center. Horizontal labels use distinct 135/90/45-degree polar anchors outside their own radii and mirror across the horizontal axis with the opening. Today uses one locale-aware K/M/B formatter for visible text while accessibility retains the exact token count. Weekly and five-hour colors use remaining thresholds of 50% and 20%; Today uses consumption thresholds of 50% and 80%. Stale metrics also use opacity, dash, visible text, and accessibility state.
