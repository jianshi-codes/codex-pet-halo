...
- Debounce sparse notifications for 250 milliseconds and refetch complete snapshots. A rate-limit update may retain same-account data as stale; an account update immediately clears all account-scoped data and requires a full account refresh. Because account identity is discarded, a new app-server connection clears old account data instead of assuming continuity.
