...
The ring uses a fixed `448×252` transparent SwiftUI vector surface whose midpoint remains the Pet visual center. Its three independent 6-point tracks retain 104/94/84-point radii and a `162`-point transparent center; the added horizontal canvas is only for labels. Horizontal Weekly/5h/Today capsules use fixed identity dots and tinted keys while progress arcs retain status-driven system colors. Their fixed-size absolute stack sits outside the rings, aligns its leading edge on the right, uses short neutral dashed connectors, and mirrors to a trailing-edge-aligned left stack when the opening flips; fixed frames and scale floors keep it inside the panel without moving the shared center. It has no material, rounded card, title container, ScrollView, in-panel control, or panel shadow. Normal mode remains pointer-transparent; only explicit center adjustment temporarily accepts drag events, without key/main eligibility.
