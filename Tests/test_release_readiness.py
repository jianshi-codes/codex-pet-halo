...
class ReleaseReadinessTests(unittest.TestCase):
    ...
    def test_release_metadata_is_numeric_and_next_beta_is_tag_only(self) -> None:
        ...
        self.assertIn('BUILD_NUMBER:-7', common)
        self.assertIn("v0.1.0-beta.7", common)
        self.assertIn("EXPECTED_BUILD_NUMBER:-6", validator)
    ...
    def test_readme_documents_both_screenshots_and_unsigned_warning(self) -> None:
        ...
        self.assertIn("Download v0.1.0-beta.6", readme)
        self.assertIn("Pet-Halo-0.1.0-beta.6-unsigned-universal.zip", readme)
    ...
    def test_beta_six_release_closeout_documents_published_state(self) -> None:
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        changelog = (ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
        ...
        self.assertIn("CURRENT_PROJECT_VERSION: 6", (ROOT / "project.yml").read_text())
        ...
        for shipped_change in (
            "transient input-method indicators",
            "Accessibility authorization settle",
            "local development build",
        ):
            self.assertIn(shipped_change, beta_six)
        self.assertTrue(beta_six.lstrip().startswith("- 2026-09-15"))
        self.assertIn("39a0c1a596309f6ff5467be65f5059f175c03c47", beta_six)
        ...
        for shipped_change in (
            ...
            self.assertIn(shipped_change, beta_four)
        self.assertTrue(beta_four.lstrip().startswith("- 2026-07-31"))
        self.assertIn("beb0c2c925d04fccf650205a611a1a20d22ead75", beta_four)
        ...
        for shipped_change in (
            ...
            self.assertIn(shipped_change, beta_three)
        self.assertTrue(beta_three.lstrip().startswith("- 2026-07-25"))
        self.assertIn("e8480a0443783e05dd871f5c248157633a84d9c5", beta_three)
        ...
        self.assertIn("Published release: `v0.1.0-beta.6`", current_state)
        ...
        self.assertIn("## Published Beta 6 record — 2026-09-15", checklist)
        self.assertIn("- [x] PR #25 merged", checklist)
        self.assertIn("34870879323", checklist)
        future = checklist.split("## Source and compatibility for a future Beta", maxsplit=1)[1]
        self.assertIn("v0.1.0-beta.7", future)
        self.assertIn("BUILD_NUMBER=7", future)
        self.assertIn("Pet-Halo-0.1.0-beta.7-unsigned-universal.zip", future)
        self.assertNotIn("BUILD_NUMBER=6", future)
    ...
    def test_beta_six_release_notes_are_frozen_and_unsigned(self) -> None:
        ...
        self.assertNotIn("Beta 6 was published", notes)
        self.assertNotIn("Beta 6 is published", notes)
        self.assertEqual(
    ...
    def test_beta_three_release_notes_are_frozen_and_unsigned(self) -> None:
        notes_path = ROOT / "docs/release-notes/v0.1.0-beta.3.md"
        notes_bytes = notes_path.read_bytes()
        notes = notes_bytes.decode("utf-8")
        checklist = (ROOT / "docs/RELEASE_CHECKLIST.md").read_text(
            encoding="utf-8"
        ...
        self.assertEqual(
            hashlib.sha256(notes_bytes).hexdigest(),
            "124e068996746f4e3b4a86193ff5e54a12df549e427f81f2b8ead4639271fa4e",
        )
        self.assertIn("## Published Beta 3 record — 2026-07-25", checklist)
        self.assertIn("v0.1.0-beta.3", checklist)
        self.assertIn("signing: unsigned", checklist)

    def test_beta_four_release_notes_are_frozen_and_unsigned(self) -> None:
        notes_path = ROOT / "docs/release-notes/v0.1.0-beta.4.md"
        notes_bytes = notes_path.read_bytes()
        notes = notes_bytes.decode("utf-8")
        checklist = (ROOT / "docs/RELEASE_CHECKLIST.md").read_text(
            encoding="utf-8"
        ...
        self.assertEqual(
            hashlib.sha256(notes_bytes).hexdigest(),
            "68ef056d7f4573a43f1fba4e343b91620e04e4621a0789e9a89b0ed9f368b869",
        )
        self.assertIn("## Published Beta 4 record — 2026-07-31", checklist)
        self.assertIn("v0.1.0-beta.4", checklist)
        self.assertIn("signing: unsigned", checklist)
    ...
    def test_current_documents_have_no_stale_published_beta_claim(self) -> None:
        current_documents = "\n".join(
            (ROOT / path).read_text(encoding="utf-8")
            for path in (
                "README.md",
        ...
        self.assertNotRegex(
            current_documents,
            re.compile(
                r"Beta\s*(?:2|3|4).{0,120}(prepared|not published|unpublished|pending publication|publication pending)",
    ...
    def test_unsigned_preview_artifact_name_and_make_path_are_explicit(self) -> None:
        ...
        self.assertEqual(
            ...
            "Pet-Halo-0.1.0-beta.7-universal.zip",
        )
        makefile = (ROOT / "Makefile").read_text(encoding="utf-8")
